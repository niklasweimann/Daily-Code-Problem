# Copied from https://scammingthecodinginterview.com/coding_problems/005.html
...
def get_files_that_are_different(root_1, root_2, file_changes_for_system_1, file_changes_for_system_2):
    ...
    if root_1.hash != root_2.hash:
        ...
        if len(root_1_children) > len(root_2_children):
            root_2_children = root_2_children + \
                [None for i in range(0, len_diff)]
        elif len(root_2_children) > len(root_1_children):
            root_1_children = root_2_children + \
                [None for i in range(0, len_diff)]

        for child_1, child_2 in zip(root_1_children, root_2_children):
            get_files_that_are_different(
                child_1, child_2, file_changes_for_system_1, file_changes_for_system_2)
